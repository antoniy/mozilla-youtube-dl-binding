...
function play_video(url, arg_mpv) {
    ...
    if (url.indexOf("twitch.tv")> -1) { // URL is Twitch link
        if (arg_mpv == "sound") { // play audio only
            var args = "--ytdl-format=audio_only".split(" "); // Twitch settings for play audio only
        }
        else { 
            var args = simple_prefs.prefs.twitch_params.split(" ");
        }
    } else if (arg_mpv == "sound") { // play audio only
        var args = simple_prefs.prefs.snd_params.split(" ");
    } else {
...
var menuItem_snd = contextMenu.Item({
  label: "Play audio with MPV",
  context: contextMenu.SelectorContext("[href]"),
  contentScript: 'self.on("click", function(node,data){self.postMessage(node.href);})',
  image: data.url("icon_button.png"),
  onMessage: function (url) {
    play_video(url, "sound");
  }
});
